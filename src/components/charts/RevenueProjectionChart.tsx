...
function CustomTooltip({
  ...
  const dataPoint = payload[0]?.payload as RevenueProjection | undefined
...
export function RevenueProjectionChart({
  ...
  const handleClick = useCallback(
    (data: { activePayload?: Array<{ payload: RevenueProjection }> }) => {
          ...
          <ComposedChart
            ...
            onMouseMove={(state) => {
              if (state?.activePayload?.[0]?.payload) {
                setHoveredYear((state.activePayload[0].payload as RevenueProjection).year)
              }
            }}
